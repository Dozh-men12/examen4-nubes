...
app.post('/register', (req, res) => {
  ...
  axios
    ...
    .then(response => {
      ...
      if (success) {
        const query = 'INSERT INTO usuarios (nombre, apellido, gmail, password) VALUES (?, ?, ?, ?)';
...
app.post('/login', async (req, res) => {
  ...
  axios
    ...
    .then(response => {
      ...
      if (success) {
        const query = 'SELECT * FROM usuarios WHERE gmail = ?';
          ...
          if (err) {
            ...
            if (results.length === 0) {
              ...
              if (user.intentos_fallidos >= 3) {
                // Cuenta bloqueada
                res.render('login', { error: 'ATENCION!!', accountBlocked: true, siteKey });
              } else if (user.password !== password) {
                // Credenciales incorrectas
                const intentosFallidos = user.intentos_fallidos + 1;
                const updateQuery = 'UPDATE usuarios SET intentos_fallidos = ? WHERE id = ?';
                ...
                const resetQuery = 'UPDATE usuarios SET intentos_fallidos = 0 WHERE id = ?';
